...
(0, _flatpickrDefault.default)(inputDate, options);
function checkedCorrectDate(date) {
    const targetTime = deadlineTime(date);
    if (targetTime <= 0) return (0, _notiflixDefault.default).Notify.failure("Please choose a date in the future");
...
function onClick() {
    const intervalTimer = setInterval(()=>{
        ...
        if (timeLeft <= 1000) clearInterval(intervalTimer);
